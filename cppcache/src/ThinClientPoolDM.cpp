...
void ThinClientPoolDM::cleanStaleConnections(std::atomic<bool>& isRunning) {
  ...
  // TODO. GEODE-8601. Usage of external reset is totally not recommended
  //       as it has an undefined behavior. Consider if its work it to
  //       re-schedule the task, or just wait for the next re-trigger.

  //  if (conns_mgmt_task_id_ != ExpiryTask::invalid() && isRunning &&
  //      m_connManager.getCacheImpl()->getExpiryTaskManager().resetTask(
  //          conns_mgmt_task_id_, _nextIdle)) {
